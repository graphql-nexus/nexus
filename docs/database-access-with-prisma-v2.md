...
#### 1.4. Tweak generated project

Replace the default script with a `main.ts` that we will be using:

```bash
rm src/script.ts
touch src/main.ts
```
Update our `package.json` including a hand `dev` script:

```diff
  "scripts": {
+    "dev": "ts-node-dev --no-notify --respawn --transpileOnly src/main.ts",
