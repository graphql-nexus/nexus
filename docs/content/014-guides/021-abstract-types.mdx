...
When DMF (`__typename`) and Centralized (`resolveType`) strategies are both enabled then `__typename` becomes optional. This is because Nexus no longer knows in a given case which strategy you will definitely use at runtime. For example:
    ...
    t.field('search', {
      ...
      resolve(_, args, ctx) {
        return ctx.db.search(args.pattern).map(result => {
          return {
            ...result,
            __typename: 'Photo', // <-- This property is __OPTIONAL__
          }
        })
      },
    })
...
unionType({
  name: 'SearchResult',
  resolveType(data) {
    if (data.__typename /* <-- This property is __OPTIONAL__ */) {
...
This behaviour is a consequence of the fact that Nexus' abstract types settings are currently global. In the future however ([#623](https://github.com/graphql-nexus/schema/issues/623)) there may be ways to enable a particular strategy for only a subset of your schema. This way you could have, for example, DMF (`__typename`) required for one abstract type so that resolvers for fields of that type would be required to include `__typename` in the returned data, while another abstract type use the Centralized stratey.

Here's that example in code:

> Note the following is just for illustration purposes. Not an API that has been committed to.
...
unionType({
  ...
  resolveType(data) { // <-- Optional method b/c strategy is DMF
    // Re-implementation of the default resolveType behaviour
    // for demonstration purposes only.

    return data.__typename // <-- This property is __GURANTEED__
  },
  definition(t) {
    t.members('Photo' /* ... */)
  },
...
unionType({
  name: 'SomethingElse'
  strategy: 'Centralized', // <-- Set the strategy only for this abstract type
  resolveType(data) { // <-- Required method because strategy is DMF
    data.__typename // <-- Type error: This property __DOES NOT EXIST__
    // ...
  },
  definition(t) {
    t.members(/* ... */)
