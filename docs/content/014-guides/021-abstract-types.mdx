...
The Centralized strategy allows to discriminate your union member types in a centralized (to the union type) way. For example:
...
The Modular strategy allows you to discriminate your union member types in a modular way like the DMF strategy. Unlike the DMF strategy however it is not based on the shape of your underlying model data. Instead the IsTypeOf strategy uses a predicate function that you implement that allows Nexus (actually GraphQL.js under the hood) to know at runtime if data being sent to the client is of the respective type or not. Here is an example:
...
Like with the ResolveType and DMF strategies Nexus leverages TypeScript to ensure your implementation is correct in the following ways:
...
### Runtime Checks Picking Your Strategy (Or Strategies)
...
2. Discriminant Model Field (`__typename`)
3. Modular (`isTypeOf`)
