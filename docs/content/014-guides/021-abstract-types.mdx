...
When DMF (`__typename`) and Centralized (`resolveType`) strategies are both enabled then `__typename` becomes an optional model field on all members of all abstract types _that **do** implement `resolveType`_. All members of abstract types that _do **not** implement_ `resolveType` will have their `__typename` model property be _required_. The reason for this behaviour is that when both strategies are enabled and an abstract type has `resolveType` implemented Nexus cannot statically know what your implementation is actually doing and if `__typename` should actually be required for it.
...
unionType({
  name: 'SearchResult',
  resolveType(data) { // <-- OPTIONAL; If given, __typename becomes OPTIONAL
