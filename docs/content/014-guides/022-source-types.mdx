...
By default, Nexus will automatically map the source type `User` to a GraphQL Object type also named `User`. Let's see how that works:
...
const User = objectType({
  name: 'User',
  //      ^---------------------- Name of your GraphQL Object type which will be matched against the name of your source types
  definition(t) {
    t.string('fullName', {
      resolve(user) {
        //      ^------------------------ as: User
...
#### Different names following a convention
...
In that case, you can configure your `source` to teach Nexus about your convention using regexes:
...
You can also use multiple regexes to map your source types by returning an array of regexes:
...
However, if _none of your source type names can match your GraphQL Object type names_ using a RegEx, we suggest you to use the [local source types configuration](#locally-configure-backing-types).
...
export const User = objectType({
  ...
  rootTyping: {
    ...
    name: 'MyDBUser'
    //         ^---------------------- Tell Nexus the name of the source type.
  },
  definition(t) {
    t.string('fullName', {
      resolve(user) {
